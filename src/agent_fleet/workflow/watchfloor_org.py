...
class WatchfloorOrganization:
    ...
    def __post_init__(self) -> None:
        ...
        # Only EXEC-1 may emit execution reports; proposers may emit proposals
    ...
    def validate_proposal(self, proposal: TradeProposal) -> ValidationResult:
        agent = next((a for a in self.registry["agents"] if a["id"] == proposal.strategy_agent_id), None)
        checks = {
        ...
        # Discretionary proposers must carry thesis; quant wing may skip
    ...
    def portfolio_risk(self, proposal: TradeProposal) -> tuple[PortfolioVerdict, RiskVerdict]:
        ...
        # Watchfloor default comparable sizing is tiny; clamp suggestions for paper NAV %
        size = min(size, 2.0)
        reasons: list[str] = []
        ...
        if name_exp > 5.0:
            ...
            size = max(0.0, 5.0 - self.name_exposure.get(proposal.symbol, 0.0))
    ...
    def governance(self, proposal: TradeProposal, steward: RiskVerdict) -> RiskVerdict:
        ...
        # Mode B: HUMAN-1 auto-passes below threshold (size <= 2% NAV)
    ...
    def propose_from_watchfloor_agent(
        ...
        *,
        ...
        hold_max = int(agent.get("holding_cap_days") or 5)
        if agent.get("holding_period") == "intraday":
            hold_min, hold_max = 0, 1
        else:
            hold_min = 1
        proposal = TradeProposal(
            strategy_agent_id=agent_id,
            strategy_version="watchfloor-0.2.0",
            symbol=symbol,
            side=side,
            thesis=thesis,
            setup_rules_fired=[f"WATCHFLOOR:{agent_id}"],
            entry_price_target=price,
            stop_loss=stop,
            invalidation_rules=["thesis_break", "mandate_breach", "halt"],
            suggested_size_pct_nav=1.0,
            holding_period_days_min=hold_min,
            holding_period_days_max=hold_max,
            valid_regimes=[regime],
            current_regime=regime,
            required_data_inputs=["tape", "thesis_log"],
    ...
    def run_approval_chain(self, proposal: TradeProposal) -> dict[str, Any]:
        ...
        track(
            MessageEnvelope(
                ...
                correlation_id=correlation,
                payload={"validation": validation.model_dump(mode="json"), "proposal": proposal.model_dump(mode="json")},
            )
        )
        ...
        # Map approving agent labels already use Watchfloor IDs from verdicts
        track(
            MessageEnvelope(
        ...
        if decision.execution_authorized and decision.status in {
            ...
            if fill.status == "filled" and decision.final_size_pct_nav:
                ...
                self.name_exposure[proposal.symbol] = (
                    self.name_exposure.get(proposal.symbol, 0.0) + decision.final_size_pct_nav
                )
